...
def register_user(traveluser: UserRegisterSchema, db: Session = Depends(get_db)) -> TravelUserOutSchema:
    hashed_password: str = hash_password(traveluser.hashed_password)
    traveluser.hashed_password = hashed_password
    try:
        new_user = TravelUser(**traveluser.model_dump())
        db.add(new_user)
        db.commit()
